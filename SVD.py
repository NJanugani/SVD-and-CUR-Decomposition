...
u,s,v = svd(arr)
arr = np.asarray(arr)
k = np.matmul(u,np.matmul(np.diag(s),np.transpose(v)))
error = 0.0
error = frob(arr,k)
print error
np.savetxt(r'C:\Users\Nishanth\Desktop\svd_original.csv',np.asarray(arr),delimiter=" ")
np.savetxt(r'C:\Users\Nishanth\Desktop\svd_computed.csv',k,delimiter=" ")
print "Done..!"
csvfile.close()