...
def cur(p):#Calculates CUR decomposition
    ...
    tot=0;
    for i in range(m):#Calculate probabilities for each row and column
        for j in range(n):
                row[i]+=(p[i][j]*p[i][j])
                tot+=(p[i][j]*p[i][j])
    for i in range(n):
    ...
    for i in range(n):
        col[i]/=tot
    rowa = np.asarray(row)#Sort rows and columns based o probability
    ...
    num_c = rd.randrange(1,n+1)#Generate random number for selection of rows and columns
    num_r = rd.randrange(num_c,m+1)
    row_mat = row_mat[:num_r,:]#Select rows and columns and modify their values
    ...
    #print "Number of rows randomly generated",num_r
    #print "number of columns randomly generated",num_c
    i=0
    j=0
    lis=[]
    for i in range(num_r):#Intersection matrix of C and R
    ...
    final_u = np.linalg.pinv(arr_u)#Pseudo-inverse of intersection matrix
...
error = frob(arr,k)
print error
np.savetxt(r'C:\Users\Nishanth\Desktop\cur_original.csv',np.asarray(arr),delimiter=" ")
np.savetxt(r'C:\Users\Nishanth\Desktop\cur_computed.csv',k,delimiter=" ")
print "Done..!"
csvfile.close()